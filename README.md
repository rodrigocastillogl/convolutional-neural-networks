...
**Locality**. Finally, adding locality, when we compute the value $\mathbf{H}{i,j}$ we do not have to consider 
pixels in the input that are far away from the location $(i,j)$. This means that outside the range $|a|<\Delta$, 
$|b|<\Delta$ we should set $\mathbf{V}{a,b}=0$, or equivalently
